...
global COL_USR_BIASUIRE_ENUB COL_USR_SIG2UIRE_ENUB
...
iono_cov_enub = nan(nUser, nDim^2, ntstep);
...
while tcurr<=tend
    ...
    % USER processing
    [vhpl, usr2satdata, Uire, usrdata] = usrprocess(satdata, usrdata, igpdata, ...
                               inv_igp_mask, usr2satdata, usrtrpfun, ...
                               usrcnmpfun, tcurr, pa_mode, give_mode, ...
                               rss_udre, rss_iono);
    iono_mean_enub(:, :, itstep) = usrdata(:, COL_USR_BIASUIRE_ENUB);
    iono_sig2_enub(:, :, itstep) = usrdata(:, COL_USR_SIG2UIRE_ENUB);
%     iono_cov_enub(:, :, itstep) = usrdata(:, COL_USR_COVUIRE_ENUB);
...
% % Average mean and var over all times
% avg_iono_mean_enu = nanmean(iono_mean_enu, 3);
% avg_iono_sig2_enu = nanmean(iono_sig2_enu, 3);
% % If mean and var are nan for all times, set it as nan
% allNanInd = all(isnan(iono_mean_enu), [2 3]);
% avg_iono_mean_enu(allNanInd, :) = nan;
% allNanInd = all(isnan(iono_sig2_enu), [2 3]);
% avg_iono_sig2_enu(allNanInd, :) = nan;
...
save 'outputs' satdata usrdata wrsdata igpdata inv_igp_mask sat_xyz udrei ...
               ...
               Uire iono_mean_enub iono_sig2_enub;
% OUTPUT processing
outputprocess(satdata,usrdata,wrsdata,igpdata,inv_igp_mask,sat_xyz,udrei,...
              ...
              pa_mode,udre_hist,give_hist,udrei_hist,givei_hist, Uire, ...
